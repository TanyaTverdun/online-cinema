...
namespace onlineCinema.Mapping
{
    ...
    public partial class AdminPaymentMapping
    {
        ...
        [MapProperty(nameof(PagedResultDto<PaymentAdminDto>.Items),
            nameof(PaymentListViewModel.Payments))]
        public partial PaymentListViewModel ToListViewModel(
            PagedResultDto<PaymentAdminDto> pagedDto);
            ...
            DateTime? date, string? successMessage)
        {
            ...
            for (int i = 0; i < pagedDto.Items.Count(); i++)
            {
                vm.Payments[i].IsRefundable =
                    pagedDto.Items.ElementAt(i).Status == PaymentStatus.Completed;
            }
